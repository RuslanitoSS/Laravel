...
class CommentController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($comment->save()) {
            ...
            foreach ($readers as $email) {
                Mail::to($email)->send(new NewCommentMail($comment, );
            }
            ;
    ...
    public function destroy(Comment $comment)
    {
        Gate::authorize('update_comment', $comment);
        if ($comment->delete())
            return redirect()->route('article.show', $comment->article_id)->with('status', 'Delete comment success');
        else
            return redirect()->route('article.show', $comment->article_id)->with('status', 'Delete comment failed');
